...
Respond in valid JSON format only. No markdown, no explanation.`;
     ...
     */
    detectDomainBoundaries(scanResult) {
    ...
    buildAnalysisPrompt(scanResult, samples) {
...

{
  ...
  "summary": "One paragraph describing the repository's architecture and purpose"
    ...
    parseAnalysisResponse(response, scanResult) {
        try {
    ...
    generateFallbackAnalysis(scanResult) {
        ...
        const tools = this.detectToolsFromConfig(scanResult);
    ...
    detectToolsFromConfig(scanResult) {
        const tools = [];
        ...
        if (scanResult.language === "TypeScript" || scanResult.language === "JavaScript") {
            if (scanResult.configFiles.includes("package.json")) {
                tools.push({ name: "install", command: "npm install", description: "Install dependencies" }, { name: "build", command: "npm run build", description: "Build the project" }, { name: "test", command: "npm test", description: "Run tests" }, { name: "lint", command: "npm run lint", description: "Run linter" });
            }
        }
        else if (scanResult.language === "Python") {
            tools.push({ name: "install", command: "pip install -e .", description: "Install dependencies" }, { name: "test", command: "pytest", description: "Run tests" }, { name: "lint", command: "ruff check .", description: "Run linter" });
        }
        else if (scanResult.language === "Go") {
            tools.push({ name: "build", command: "go build ./...", description: "Build the project" }, { name: "test", command: "go test ./...", description: "Run tests" }, { name: "lint", command: "golangci-lint run", description: "Run linter" });
        }
        return tools;
    }
    generateDefaultHooks(scanResult) {
        const hooks = [];
