...
export async function assimilateCommand(target, options) {
    ...
    if (isRemote) {
        ...
        console.log(chalk.green("\n[GENERATE]"), options.dryRun ? "Preview of assets..." : `Writing assets to ${outputPath}/.github/...`);
        // Generate
        ...
        console.log(chalk.green("\n[COMPLETE]"), `${result.skills.length} skills, ${primaryCount} agents, ${subAgentCount} sub-agents, ${result.hooks.length} hooks generated.`);
        if (options.dryRun) {
...
async function assimilateLocal(target, options) {
    ...
    try {
        ...
        console.log(chalk.green("\n[GENERATE]"), options.dryRun ? "Preview of assets..." : `Writing assets to .github/...`);
        const generator = new Generator(outputPath, options.dryRun, options.verbose);
        ...
        console.log(chalk.green("\n[COMPLETE]"), `${analysisResult.skills.length} skills, ${primaryCount} agents, ${subAgentCount} sub-agents, ${analysisResult.hooks.length} hooks generated.`);
        if (options.dryRun) {
            console.log(chalk.yellow("\nDry run - no files were written."));
        }
        else {
