    ...
    try {
        ...
        for (const dir of agentDirs) {
            ...
            try {
                ...
                if (agent.skills && Array.isArray(agent.skills)) {
                    for (const skillName of agent.skills) {
                        ...
                        try {
                            ...
                            result.warnings.push(`${dir.name}/agent.yaml: References non-existent skill '${skillName}'`);
                        }
                    }
                }
                if (verbose) {
                ...
                result.errors.push(`${dir.name}/agent.yaml: Invalid YAML - ${e.message}`);
                result.valid = false;
            }
        }
        ...
        for (const file of hookFiles) {
            ...
            try {
                ...
                if (!hook.event) {
                    ...
                    result.errors.push(`hooks/${file}: Invalid event '${hook.event}'. Must be one of: ${validEvents.join(", ")}`);
                    result.valid = false;
                }
