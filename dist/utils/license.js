...
export async function detectLicense(repoPath) {
    ...
    try {
        ...
        if (pkg.license) {
            ...
            const isPermissive = Object.keys(PERMISSIVE_LICENSES).some((key) => key.toLowerCase() === spdxId.toLowerCase());
            return {
                detected: true,
                name: spdxId,
                spdxId: spdxId,
    ...
    try {
        ...
        if (licenseMatch) {
            ...
            const isPermissive = Object.keys(PERMISSIVE_LICENSES).some((key) => key.toLowerCase() === licenseName.toLowerCase());
            return {
                detected: true,
                name: licenseName,
                spdxId: licenseName,
...
export function formatLicenseStatus(license) {
    if (!license.detected) {
